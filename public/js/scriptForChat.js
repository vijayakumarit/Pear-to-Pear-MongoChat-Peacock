$ (function(){
  ...
  socket.on('onlineStack',function(stack){
    ...
    for (var user in stack){
      ...
      if(stack[user] == "Online"){
        var txt2 = $('<span class="badge"></span>').text("*"+stack[user]).css({"float":"right","color":"rgb(255, 255, 255)","font-size":"18px"});
        totalOnline++;

      }
      else{
        var txt2 = $('<span class="badge"></span>').text(stack[user]).css({"float":"right","color":"#ffffff","font-size":"18px","background-color":"red"});
      }
      //listing all users.
  ...
  $('form').submit(function(){
    socket.emit('fileupload-msg',{msg:$('#myMsgfile').val(),msgTo:toUser,date:Date.now()});
    $('#myMsgfile').val("");
    $('#sendBtn').hide();
    return false;
  }); //end of sending message.
  ...
  socket.on('chat-msg',function(data){
    ...
    console.log("MESSAGE JQUERY CHAT",data.msg)
    //showing chat in chat box.
  ...
  socket.on('fileupload-msg',function(data){
    //styling of chat message.
    var chatDate = moment(data.date).format("MMMM Do YYYY, hh:mm:ss a");
    var txt1 = $('<span></span>').text(data.msgFrom+" : ").css({"color":"#006080"});
    var txt2 = $('<span></span>').text(chatDate).css({"float":"right","color":"#a6a6a6","font-size":"16px"});
