...
module.exports.sockets = function(http) {
  ...
  ioChat.on("connection", function(socket) {
    ...
    // console.log("DATA FIRST ONE ",data.msg)
     if(data.msg == ""){
       console.log(" Text message Data is empty")
     }
     else {
...
//for showing File Uplaods

socket.on("fileupload-msg", function(data) {

  //console.log("CHAT MESSAGE",data)
  if(data.msg ==""){
    console.log("File Upload Data is Empty")
  }
  else {
    //emits event to save chat to database.
  eventEmitter1.emit("save-chat-upload", {
    msgFrom: socket.username,
    msgTo: data.msgTo,
    msg: data.msg,
    room: socket.room,
  ...
  //emits event to send chat msg to all clients.
  ioChat.to(socket.room).emit("fileupload-msg", {
    msgFrom: socket.username,
    msg: data.msg,
    date: data.date
  });
  ...
  eventEmitter1.on("save-chat-upload", function(data) {
    // var today = Date.now();
